...
public class DownloadCredentialsWizard extends DialogWrapper {
    ...
    protected DownloadCredentialsWizard(AutonomousDatabaseSummary autonomousDatabaseSummary) {
        super(true);
        this.autonomousDatabaseSummary = autonomousDatabaseSummary;
        this.isDedicatedInstance = (autonomousDatabaseSummary.getIsDedicated() != null
    ...
    private void initializeWalletTypes() {
        ...
        walletTypeCmb.addItemListener((e) -> {
            ...
            if(selectedWalletType.equals(ADBConstants.INSTANCE_WALLET))
                rotationTimeStr = (instanceWallet != null && instanceWallet.getTimeRotated() != null) ? instanceWallet.getTimeRotated().toGMTString() : "Not Available";
            else
                rotationTimeStr = (regionalWallet != null && regionalWallet.getTimeRotated() != null) ? regionalWallet.getTimeRotated().toGMTString() : "Not Available";
        ...
        final String rotationTimeStr = (instanceWallet != null && instanceWallet.getTimeRotated() != null) ? instanceWallet.getTimeRotated().toGMTString() : "Not Available";
        rotateWalletLbl.setText("Wallet Last Rotated Time : " + rotationTimeStr);
        ...
        rotateWalletBtn.addActionListener((e) -> {
            ...
            final String msg;
            if(selectedWalletType.equals(ADBConstants.INSTANCE_WALLET))
                msg = rotationMsgInstanceWallet + String.format(rotationMsg1, autonomousDatabaseSummary.getDbName(), "INSTANCE WALLET");
            else
                msg = rotationMsgRegionalWallet + String.format(rotationMsg1, autonomousDatabaseSummary.getDbName(), "REGIONAL WALLET");

            final String input = Messages.showInputDialog(msg,"Rotate Wallet", Messages.getQuestionIcon());
            if(autonomousDatabaseSummary.getDbName().equals(input)) {
                ADBInstanceClient.getInstance().rotateWallet(autonomousDatabaseSummary.getId(), selectedWalletType);
                Messages.showInfoMessage("Wallet rotated successfully", "Success");
                close(DialogWrapper.CLOSE_EXIT_CODE);
            }
            else {
                Messages.showErrorDialog("Entered database name does not match with existing name.", "Database name mismatch error");
            }
        });
    }
    ...
    public void doOKAction() {
        ...
        final String adminPassword = new String(passwordPasswordField.getPassword());
        final String selectedWalletType = (String)walletTypeCmb.getSelectedItem();
        final String walletDirectory = dirPath+File.separator+"Wallet_" + autonomousDatabaseSummary.getDbName();

        final Runnable nonblockingDownload = () -> {
            try {
                ADBInstanceClient.getInstance().downloadWallet(
                    autonomousDatabaseSummary, selectedWalletType,
                    adminPassword, walletDirectory);
                ApplicationManager.getApplication().invokeLater(() -> UIUtil
                    .fireSuccessNotification("<html>Wallet downloaded successfully."
                        + "<br>Path : " + walletDirectory
                        + "</html>"));
            }
            catch (Exception e) {
                ApplicationManager.getApplication().invokeLater(
                    () -> UIUtil.fireErrorNotification("Download wallet failed."));
            }
        };
    ...
    private boolean isValidPassword() {
        ...
        final String confirmPassword = new String(confirmPasswordPasswordField.getPassword());
